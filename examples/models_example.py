...
def example_classification():
    ...
    # 创建示例数据
    batch = {"features": torch.randn(32, 128)}  # (batch_size, input_dim)
...
def example_finetune():
    ...
    # 创建模型
    model = get_model("mlp_classifier", task_type="classification", input_dim=128, hidden_dims=[256, 128, 64], num_classes=10, dropout=0.1)
...
def example_direct_instantiation():
    ...
    # 直接实例化分类模型
    classifier = MLPClassifier(input_dim=128, hidden_dims=[256, 128], num_classes=5, dropout=0.2)

    print(f"模型类型: {classifier.__class__.__name__}")
    print(f"类别数: {classifier.num_classes}")

    # 直接实例化回归模型
    regressor = MLPRegressor(input_dim=128, hidden_dims=[256, 128], num_outputs=3, dropout=0.2)  # 多输出回归
