...
class HEPDataset(IterableDataset):
    ...
    def _load_and_preprocess(self) -> tuple:
        ...
        # 1. 确定要加载的分支
        load_branches = (
            self.data_config.train_load_branches
            if self.for_training
            else self.data_config.test_load_branches
        )
        ...
        # 3. 应用选择条件
        selection = (
            self.data_config.selection
            if self.for_training
            else self.data_config.test_time_selection
        )
        ...
        table = _apply_selection(
            table,
            selection,
            funcs=self.data_config.var_funcs
        )
        ...
        # 4. 构建新变量
        aux_branches = (
            self.data_config.train_aux_branches
            if self.for_training
            else self.data_config.test_aux_branches
        )
