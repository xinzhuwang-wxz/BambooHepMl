...
from bamboohepml.data import DataConfig, DataSourceFactory  # noqa: E402
from bamboohepml.data.preprocess import AutoStandardizer  # noqa: E402
from bamboohepml.data.tools import _clip, _pad, _repeat_pad  # noqa: E402
...
def test_auto_standardization():
    ...
    try:
        ...
        if "part_energy" in data_config.preprocess_params:
            ...
            print("  ⚠️  part_energy 不在 preprocess_params 中")
        ...
        if "part_energy" in data_config.preprocess_params:
            if data_config.preprocess_params["part_energy"].get("center") != "auto":
                ...
                print("  ✓ 已设置 part_energy.center = 'auto'")
            else:
                print("  ✓ part_energy.center 已经是 'auto'")
        else:
            print("  ⚠️  无法设置 part_energy.center，因为不在 preprocess_params 中")
        ...
        for k, params in data_config.preprocess_params.items():
            if params.get("center") != "auto" and params.get("center") is not None:
                ...
                print("    ✓ 参数已从 'auto' 更新为实际值")

        print("\n✓ 自动标准化功能正常")
        print("  现在可以在验证集/测试集上使用相同的参数")
